...
def init_kedro(path, env: str = None, extra_params: Dict[str, Any] = None):
    ...
    try:
        ...
        # `del` is used instead of `reload()` because: If the new version of a module does
        # not define a name that was defined by the old version, the old definition
        # remains.
        ...
        logging.info(
            "Defined global variable `context`, `session` and `catalog`"
        )
...
if __name__ == "__main__":
    ...
    init_kedro(project_path, env=args.env)
    session.run(
        runner=ThreadRunner(),
        node_names=[node_name.strip() for node_name in args.nodes.split(",")],
    )
